...
describe('cadence representative lesson', () => {
  ...
  it('hides harmonic analysis until the listener answers', () => { const block = lesson?.blocks.find((item) => item.id === 'harmony.block.cadence.listener'); const input = (block?.data as { input: { hideAnalysisUntilAnswer?: boolean } }).input; expect(input.hideAnalysisUntilAnswer).toBe(true); });
