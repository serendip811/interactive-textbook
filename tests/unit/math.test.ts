...
describe('math meaning data', () => {
  ...
  it('returns actionable validation feedback', () => { expect(validatePoint({ x: 2, y: 3 }, { x: 2, y: 1 }).message).toContain('x좌표는 맞습니다'); expect(validateLinearParameters(line('target', 2, 1), line('selected', 2, 0)).message).toContain('기울기는 맞습니다'); expect(validateIntersection({ kind: 'point', point: { x: 1, y: 1 } }, { x: 1, y: 1 }).correct).toBe(true); });
});
