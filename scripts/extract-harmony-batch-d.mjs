...
const outputPath = resolve(root, 'migration/harmony-batch-d-extract.json');
...
while ((match = pattern.exec(html))) {
  ...
  lessons.push({
    ...
    sequences: [...body.matchAll(/data-sequence='([^']+)'/g)].map((item) => JSON.parse(item[1])),
    evidence: {
...
if (lessons.length !== 17) throw new Error(`Batch D must contain 17 lessons, found ${lessons.length}`);
mkdirSync(dirname(outputPath), { recursive: true });
writeFileSync(outputPath, `${JSON.stringify({ source: 'Harmony RC1', batch: 'D', parts: [10, 11, 12, 13], lessons }, null, 2)}\n`);
