...
export function Keyboard({ selected, activeMidi, onSelect, fromMidi = 60, toMidi = 72 }: { selected: Pitch[]; activeMidi?: number; onSelect?: (pitch: Pitch) => void; fromMidi?: number; toMidi?: number }) {
  ...
  return <div className="keyboard" role="group" aria-label={`${pitchName(midiToPitch(fromMidi))}부터 ${pitchName(midiToPitch(toMidi))}까지 피아노 건반`}>{keys.map((key) => { const selectedIndex = selected.findIndex((pitch) => pitchToMidi(pitch) === key.midi); const style = key.black ? { left: `${(key.whiteIndex + .72) * width}%`, width: `${width * .56}%` } : { left: `${key.whiteIndex * width}%`, width: `${width}%` }; return <button type="button" key={key.midi} className={`piano-key ${key.black ? 'black' : 'white'} ${selectedIndex >= 0 ? 'selected' : ''} ${activeMidi === key.midi ? 'playing' : ''}`} style={style} aria-label={`${pitchName(key.pitch)}${selectedIndex >= 0 ? `, ${selectedIndex + 1}번째 선택` : ''}`} aria-pressed={selectedIndex >= 0} onClick={() => onSelect?.(key.pitch)}><span>{key.black ? pitchName(key.pitch).replace(/\d/g, '') : key.pitch.step}</span>{selectedIndex >= 0 && <b>{selectedIndex + 1}</b>}</button>; })}</div>;
}
