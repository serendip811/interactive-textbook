...
export function VoiceLeadingActivity({ title, input, onComplete }: { title: string; input: { value: VoiceLeading; targetPitch?: Pitch; instruction?: string }; onComplete?: (value: unknown) => void }) {
  ...
  return <section className="music-activity"><p className="section-label">MUSIC · VOICE LEADING</p><h2>{title}</h2><p className="activity-instruction"><strong>목표</strong> {input.instruction ?? '시작음 C5에서 한 단계 아래로 내려가, 베이스와 반대 방향으로 움직이세요.'}</p><div className={issues.length ? 'voice-frame has-error' : 'voice-frame'}><Staff groups={[[upper.pitches[0], lower.pitches[0]], [upper.pitches[1], lower.pitches[1]]]} /><p>{upper.name}: {upper.pitches.map(pitchName).join(' → ')}</p><p>{lower.name}: {lower.pitches.map(pitchName).join(' → ')}</p></div><Keyboard selected={[upper.pitches[1]]} onSelect={choose} fromMidi={60} toMidi={72} /><p className={feedback.startsWith('정답') ? 'play-status feedback--success' : 'play-status'} aria-live="polite">{feedback}</p></section>;
}
