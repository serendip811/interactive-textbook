...
export function App() {
  ...
  return <><a className="skip-link" href="#main-content" onClick={skipToContent}>본문으로 건너뛰기</a><div className="app-shell">
    <button className="toc-toggle" type="button" aria-controls="book-toc" aria-expanded={tocOpen} onClick={() => setTocOpen((open) => !open)}>{tocOpen ? '× 목차 닫기' : '☰ 목차 열기'}</button>
    <aside id="book-toc" className={`toc${tocOpen ? ' toc--open' : ''}`} aria-label="교재 목차"><div className="brand"><span>INTERACTIVE BOOK</span><strong>{result.book.title}</strong></div><nav className="book-switcher" aria-label="교재 선택"><a href="?book=harmony" aria-current={result.book.id === harmonyBook.id ? 'page' : undefined}>화성학</a><a href="?book=functions" aria-current={result.book.id === functionsBook.id ? 'page' : undefined}>함수와 그래프</a></nav>{result.book.parts.map((part) => <section key={part.id}><h2>{part.title}</h2><ol>{part.lessons.map((lesson) => <li key={lesson.id}><button className={lesson.id === currentId ? 'active' : ''} aria-current={lesson.id === currentId ? 'page' : undefined} onClick={() => selectLesson(lesson.id)}>{progress?.lessons[lesson.id]?.status === 'completed' ? '✓ ' : ''}{lesson.title}</button></li>)}</ol></section>)}{confirmReset ? <div className="reset-confirm" role="group" aria-label="새 학습 시작 확인"><p>현재 진행 기록을 지우고 첫 단원부터 시작할까요?</p><button className="reset-progress confirm" onClick={() => { clearBookProgress(localStorage, result.book.id); const reset = emptyBookProgress(result.book.id, result.book.contentVersion); setProgress(reset); selectLesson(firstLesson?.id ?? ''); setConfirmReset(false); }}>기록을 지우고 시작</button><button onClick={() => setConfirmReset(false)}>취소</button></div> : <button className="reset-progress" onClick={() => setConfirmReset(true)}>새 학습 시작</button>}</aside>
