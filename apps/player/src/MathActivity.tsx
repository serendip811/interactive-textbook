...
function CoordinateGraph({ plane = defaultPlane, points = [], lines = [], onSelect }: { plane?: CoordinatePlane; points?: Array<{ point: Point; label?: string; target?: boolean }>; lines?: LinearFunction[]; onSelect?: (point: Point) => void }) {
  ...
  const selectFromKeyboard = (event: KeyboardEvent<SVGSVGElement>) => {
    ...
    else if (event.key === 'ArrowDown') next.y -= plane.yAxis.step;
    else if (event.key === 'ArrowUp') next.y += plane.yAxis.step;
    else return;
    event.preventDefault();
    onSelect({ x: clamp(next.x, plane.xAxis.min, plane.xAxis.max), y: clamp(next.y, plane.yAxis.min, plane.yAxis.max) });
  };
  return <svg className={`coordinate-graph${onSelect ? ' coordinate-graph--interactive' : ''}`} viewBox={`0 0 ${width} ${height}`} role="img" tabIndex={onSelect ? 0 : undefined} aria-label={`좌표평면: ${description || '표시된 데이터 없음'}${onSelect ? '. 그래프를 누르거나 방향키로 점을 이동할 수 있습니다.' : ''}`} onPointerDown={selectFromPointer} onKeyDown={selectFromKeyboard}>
